	...
	function packFileArray($file_array){
		...
		for ($i = 0; $i<count($file_array); $i++){
			...
			if (strlen($filename)<=0)
				continue;
			if (!file_exists($filename)){$this->errors[] = __('No file').' '.$filename;
				continue;
			}
			if (!$this->tmp_file){
			$this->errors[] = __('Invalid file descriptor');
